...
├── utils/
│   ├── llm.py                  # Groq calls (streaming + non-streaming), prompt logic, question rewriting
│   ├── embedding.py             # Sentence-transformers model loading
│   ├── retriever.py             # Chunk retrieval logic (auto-scaled k)
...
│   ├── errors.py                # Friendly, non-leaky error messages for the UI
│   └── theme.py                 # Custom CSS, hero section, feature grid
├── tests/                     # pytest unit tests for the utils/ logic
...
The test suite covers the pure-logic pieces (chunking, retrieval,
vector store, the quiz-JSON parser, and the streaming fallback chain)
with mocked dependencies — it doesn't need real API keys. `ruff check .`
lints the codebase. Both run automatically on push/PR via GitHub Actions
...
`components/`. New logic in `utils/` should come with a test in `tests/`.
