...
def chat_completion_stream(messages, temperature=0.2):
    """Streaming counterpart to chat_completion(). Walks the same
    (api_key, model) fallback chain, but yields answer text
    incrementally instead of blocking for the full response — meant
    for use with st.write_stream() so the user sees tokens appear as
    they're generated instead of staring at a spinner for the whole
    call.

    The fallback logic still applies to the *initial* request: Groq's
    client raises immediately (before any streaming starts) if the
    request itself is rejected — e.g. a 429 rate limit — so a
    rate-limited (key, model) pair is caught and skipped here exactly
    like in chat_completion(), before anything is shown to the user.
    Once a stream has actually started, though, we're committed to it:
    a failure mid-stream propagates as a normal exception rather than
    silently retrying and re-streaming a duplicate answer.
    """
    chain = _get_fallback_chain()
    last_error = None
    attempted = []
...
def _build_ask_prompt(context: str, question: str, chat_history=None) -> str:
    history = ""
...
def ask_groq(context: str, question: str, chat_history=None):
    prompt = _build_ask_prompt(context, question, chat_history)
    return chat_completion([{"role": "user", "content": prompt}], temperature=0.2)


def ask_groq_stream(context: str, question: str, chat_history=None):
    """Streaming counterpart to ask_groq() — same prompt, same rules,
    yields tokens for use with st.write_stream(). Used for the
    "PDF Only" search mode and for Hybrid mode's second (web) pass,
    where the caller doesn't need to inspect the full answer before
    showing it to the user."""
    prompt = _build_ask_prompt(context, question, chat_history)
    return chat_completion_stream([{"role": "user", "content": prompt}], temperature=0.2)
...
"""
    ...
    return _parse_quiz_json(content)


def _parse_quiz_json(content: str):
    """Parse the model's quiz output into a list of question dicts.

    Models occasionally wrap JSON in a ```json fence, add a stray
    leading/trailing sentence, or otherwise produce near-JSON instead
    of strictly valid JSON. Rather than let json.loads() crash the
    whole request (which previously surfaced as an uncaught exception
    in the UI), we: strip code fences, try a direct parse, then fall
    back to extracting the outermost [...] array before giving up with
    ...
    # Fall back to grabbing the first '[' through the last ']' — covers
    # cases where the model added a sentence before/after the array.
    start = text.find("[")
    end = text.rfind("]")
    ...
    raise ValueError(
        "The quiz generator returned output that wasn't valid JSON. "
        "This is usually transient — please try generating the quiz again."
    )
...
def _build_ask_web_prompt(context: str, question: str) -> str:
    return f"""
{_question_language_instruction(question)}You are an AI Research Assistant.
...
def ask_groq_web(context: str, question: str):
    prompt = _build_ask_web_prompt(context, question)
    return chat_completion([{"role": "user", "content": prompt}], temperature=0.2)


def ask_groq_web_stream(context: str, question: str):
    """Streaming counterpart to ask_groq_web() — same prompt, same
    rules, yields tokens for use with st.write_stream()."""
    prompt = _build_ask_web_prompt(context, question)
    return chat_completion_stream([{"role": "user", "content": prompt}], temperature=0.2)
